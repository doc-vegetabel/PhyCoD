...
# 2026-05-12 static-failure-aware phase weighting update
...
# 2026-05-14 local phase-correlation loss update

| Time | Files | Change Type | Content | Stage |
|---|---|---|---|---|
| 2026-05-14 Asia/Shanghai | `src/student/transformer/frequency_losses.py` | Add | Add `local_phase_correlation_loss(...)`, an amplitude-invariant local window objective that maximizes zero-lag normalized correlation and penalizes the best-lag-vs-zero-lag correlation gap. High-frequency and static-failure weighting are computed from local signals, not from case names. | frequency/phase-first alignment |
| 2026-05-14 Asia/Shanghai | `scripts/train_transformer_physical_params_torch.py` | Add | Add CLI/config controls for the new local phase-correlation loss and wire the x/y losses into `freq_loss`. Defaults remain disabled, so existing commands are unchanged. | frequency/phase-first alignment |
| 2026-05-14 Asia/Shanghai | `scripts/train_transformer_physical_params_torch.py` | Modify | Add `local_phase_corr_*` diagnostics to `training_history.csv`, including corr0, best corr, corr gap, local lag, high-frequency weight, static-failure weight, and window counts. | frequency/phase-first alignment |

# 2026-05-14 local phase-increment consistency update

| Time | Files | Change Type | Content | Stage |
|---|---|---|---|---|
| 2026-05-14 Asia/Shanghai | `src/student/transformer/frequency_losses.py` | Add | Add `local_phase_increment_loss(...)`, which computes target-dominant local narrow-band complex phase and penalizes both absolute phase mismatch and adjacent-window phase-increment mismatch. The increment term targets cumulative high-frequency phase drift directly, while window emphasis is based on local spectral/static-failure state rather than case names. | cumulative high-frequency phase correction |
| 2026-05-14 Asia/Shanghai | `scripts/train_transformer_physical_params_torch.py` | Add | Add `--use-local-phase-increment-loss`, `--w-local-phase-absolute-x/y`, `--w-local-phase-increment-x/y`, and `--local-phase-increment-*` controls; wire the x/y absolute and increment terms into `freq_loss`. Defaults remain disabled, so existing runs are unchanged. | cumulative high-frequency phase correction |
| 2026-05-14 Asia/Shanghai | `scripts/train_transformer_physical_params_torch.py` | Modify | Extend `training_history.csv` diagnostics with `local_phase_increment_*` metrics, including absolute/increment losses, phase/increment cosine means, high-frequency/static-failure/combined weights, target frequency means, and window/increment counts. | cumulative high-frequency phase correction |

# 2026-05-15 continuous phase-lag update

| Time | Files | Change Type | Content | Stage |
|---|---|---|---|---|
| 2026-05-15 Asia/Shanghai | `src/student/transformer/frequency_losses.py` | Add | Add `local_continuous_phase_lag_loss(...)`, which projects each local response window onto a dense continuous frequency grid, automatically weights frequencies by teacher-band energy, and converts phase mismatch into an equivalent time-shift loss. This makes residual 0.02-0.06 s high-frequency lag errors much stronger than raw `1-cos(phase)` while still using state/spectrum-based weighting rather than case names. | time-shift-aware high-frequency phase correction |
| 2026-05-15 Asia/Shanghai | `scripts/train_transformer_physical_params_torch.py` | Add | Add `--use-continuous-phase-lag-loss`, `--w-continuous-phase-absolute-x/y`, `--w-continuous-phase-time-shift-x/y`, and `--continuous-phase-*` controls; wire x/y dense-frequency absolute phase and time-shift losses into `freq_loss`. Defaults remain disabled and the final physical core still receives only total `theta=[alpha_x_total, alpha_xy_total]`. | time-shift-aware high-frequency phase correction |
| 2026-05-15 Asia/Shanghai | `scripts/train_transformer_physical_params_torch.py` | Modify | Extend `training_history.csv` diagnostics with `continuous_phase_*` metrics, including equivalent lag in seconds, phase cosine, target frequency mean, high-frequency/static-failure weights, combined weights, and window counts. | time-shift-aware high-frequency phase correction |

# 2026-05-15 phase-slope and fast-mean separation update

| Time | Files | Change Type | Content | Stage |
|---|---|---|---|---|
| 2026-05-15 Asia/Shanghai | `src/student/transformer/frequency_losses.py` | Add | Add `local_phase_slope_loss(...)`, a dense-frequency local-window objective that compares adjacent-window phase-error increments rather than only absolute phase. It targets accumulated high-frequency phase drift as an equivalent local frequency/phase-slope error, with automatic spectrum/static-failure window weighting and no case-name logic. | cumulative high-frequency frequency/phase correction |
| 2026-05-15 Asia/Shanghai | `scripts/train_transformer_physical_params_torch.py` | Add | Expose `--use-local-phase-slope-loss`, `--w-local-phase-slope-x/y`, and `--local-phase-slope-*` controls; add the x/y phase-slope terms to `freq_loss`; log `local_phase_slope_*` diagnostics in `training_history.csv`. | cumulative high-frequency frequency/phase correction |
| 2026-05-15 Asia/Shanghai | `scripts/train_transformer_physical_params_torch.py` | Add | Add optional fast-branch window-mean regularization via `--w-theta-fast-window-mean`, `--w-theta-gated-fast-window-mean`, and `--theta-fast-window-mean-*`. This discourages the fast residual branch from carrying cycle-scale mean stiffness shifts, leaving average frequency calibration to `alpha_slow(t)` while preserving final total `theta=[alpha_x_total, alpha_xy_total]`. | slow/fast phase-gate separation |

# 2026-05-15 alpha-stage best-result baseline note

| Time | Files | Change Type | Content | Stage |
|---|---|---|---|---|
| 2026-05-15 Asia/Shanghai | `developer_log.md`, `code_doc.md` | Record | Record `result/train_gateopen_v2_fastnewmark` as the current best alpha-class physical-parameter baseline for frequency/phase alignment. It is the preferred reference if later alpha-only attempts cannot further reduce high-frequency late-time phase error without degrading simple/low/mid cases. | alpha_x/alpha_xy stage baseline |
| 2026-05-15 Asia/Shanghai | `result/train_gateopen_v2_fastnewmark` | Result note | Best checkpoint was epoch 160 with `best_score=2.286574e-02`. Post-test high-frequency holdout `f0p650` reached `full_x_mse_ratio_to_static=0.0379`, `full_y_mse_ratio_to_static=0.0676`, and late-window lag about `+0.030 s` in both x and y. Simple cases stayed near static (`simple_tip_fx` x ratio `0.9756`; `simple_tip_fy` y ratio `1.0089`). | alpha_x/alpha_xy stage baseline |
| 2026-05-15 Asia/Shanghai | `result/train_gateopen_v2_fastnewmark/train_config.yaml` | Result note | Training used 24 train cases and 4 valid cases from `tc_adaptphase_gateboost_complex_v1`, `alpha_x,alpha_xy`, phase-gated decomposition, temporal window 192, `d_model=64`, two temporal Transformer layers, `phase_fast_scale=1.0`, `phase_gate_init_bias=-2.5`, adaptive phase-window loss, `w_lag_x/y=0.05`, `w_adaptive_phase_x/y=0.06/0.10`, and `w_phase_gate_align=0.20`. | alpha_x/alpha_xy stage baseline |

# 2026-05-18 beta damping and four-curve evaluation update

| Time | Files | Change Type | Content | Stage |
|---|---|---|---|---|
| 2026-05-18 Asia/Shanghai | `src/student/transformer/physical_parameter_registry.py` | Add | Add `beta_damp_x` and `beta_damp_y` as bounded C-target physical parameters after `alpha_x,alpha_xy`. The ordering keeps alpha rows stable for warm-starting from the MAIN-ALPHA best checkpoint. | alpha+beta amplitude correction |
| 2026-05-18 Asia/Shanghai | `src/student/transformer/physical_templates.py` | Add | Add a y-bending directional stiffness-shape template and `damping_template_dict(...)`, which constructs `C_x_template` and `C_y_template` from the same stiffness-proportional damping scale used for `C0`. | physically meaningful damping residuals |
| 2026-05-18 Asia/Shanghai | `src/student/transformer/dynamic_physical_core_torch.py` | Modify | Extend the dynamic physical core from K-only theta templates to K/C theta templates. Dynamic damping now affects both the Newmark effective matrix and RHS damping force, while alpha-only runs remain compatible. | full-order MCK beta support |
| 2026-05-18 Asia/Shanghai | `scripts/train_transformer_physical_params_torch.py` | Add | Add partial checkpoint loading for alpha-to-beta warm starts. Shape-compatible tensors are copied directly; widened theta-head tensors copy old alpha rows and retain initialized beta rows. Add `--freeze-alpha-backbone-for-beta` to train only beta rows for conservative smoke tests. | beta smoke training workflow |
| 2026-05-18 Asia/Shanghai | `scripts/evaluate_transformer_vs_baselines.py` | Modify | Add optional `--alpha-checkpoint` comparison mode. Evaluation can now run alpha-only and alpha+beta checkpoints together and produce four-curve plots: teacher, base student, alpha-only, and alpha+beta. | result comparison workflow |
| 2026-05-18 Asia/Shanghai | `.gitignore` | Add | Ignore `data/`, `result/`, `results/`, Python caches, local virtual environments, and editor files so commits to the `add_beta` branch package only code and documentation. | repository hygiene |

# 2026-05-18 beta alpha-reference amplitude loss update

| Time | Files | Change Type | Content | Stage |
|---|---|---|---|---|
| 2026-05-18 Asia/Shanghai | `scripts/train_transformer_physical_params_torch.py` | Add | Add beta-specific `beta_loss_mode`. In auto mode, freeze-alpha beta runs roll out a detached alpha-only reference by zeroing beta damping theta rows, then optimize alpha-relative response ratios plus windowed log-RMS amplitude error instead of continuing to make frequency/phase loss the primary beta objective. | beta amplitude correction |
| 2026-05-18 Asia/Shanghai | `scripts/train_transformer_physical_params_torch.py` | Add | Add alpha-relative beta no-regression guards for response MSE, zero-lag correlation, and amplitude log-error worsening. These guards protect the frequency/phase behavior learned by alpha while still allowing damping C to focus on amplitude. | preserve alpha baseline |
| 2026-05-18 Asia/Shanghai | `scripts/train_transformer_physical_params_torch.py` | Modify | Add `*_ratio_to_alpha`, `beta_amp_*`, and `beta_alpha_guard_*` history diagnostics. Epoch print lines now show `_to_alpha` train/valid ratios when the beta objective is active. Also expose `--load-spectral-feature-dim` so beta runs can keep MAIN-ALPHA spectral feature dimensions with the matching spectral-band config. | beta diagnostics |

# 2026-05-18 beta damping gain and improvement-loss update

| Time | Files | Change Type | Content | Stage |
|---|---|---|---|---|
| 2026-05-18 Asia/Shanghai | `src/student/transformer/physical_templates.py`, `scripts/train_transformer_physical_params_torch.py` | Add | Add `beta_damp_template_gain_x/y` so beta damping templates can be scaled as `gain * damping_scale * K_template`. Defaults are 1.0, preserving older alpha/beta checkpoints unless a new run opts into stronger C-template leverage. | beta amplitude correction v3 |
| 2026-05-18 Asia/Shanghai | `scripts/train_transformer_physical_params_torch.py` | Add | Add alpha-relative amplitude improvement hinge diagnostics and controls: `--w-beta-amp-improvement`, `--beta-amp-improvement-margin`, plus weak beta damping sign regularizers `--w-beta-damp-sign-x/y` and `--beta-damp-sign-min-alpha-error`. These target the v2 failure mode where beta outputs existed but moved the high-frequency response by less than 1% of the alpha error. | beta amplitude correction v3 |
| 2026-05-18 Asia/Shanghai | `scripts/train_transformer_physical_params_torch.py` | Add | Add `--best-score-mode beta_amp`, which scores beta checkpoints using alpha-relative y/tip_y/last5_y/x response ratios plus beta guard and improvement terms, instead of selecting primarily for the old alpha-style mixed score. | beta checkpoint selection |
| 2026-05-18 Asia/Shanghai | `scripts/evaluate_transformer_vs_baselines.py` | Modify | Add alpha+beta versus alpha-only diagnostics to four-curve evaluation: response delta RMS/max, delta-to-alpha-error ratio, late RMS alpha/teacher and beta/teacher ratios, and beta_damp theta stats. The CSV writer now preserves additional per-row diagnostic columns. | beta result interpretation |

# 2026-05-18 high-frequency beta damping mechanism update
