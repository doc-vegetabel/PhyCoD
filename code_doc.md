...
## 31. 2026-05-18 beta alpha-reference amplitude loss update

| Time | Files | Required Note | Content | Stage |
|---|---|---|---|---|
| 2026-05-18 Asia/Shanghai | `scripts/train_transformer_physical_params_torch.py` | Add beta-specific amplitude objective | Add `beta_loss_mode=auto/alpha_relative_amp/standard`. In freeze-alpha beta runs, the trainer now rolls out a detached alpha-only reference by zeroing C-target beta theta rows, optimizes alpha-relative response ratios and windowed log-RMS amplitude error, and keeps frequency/phase losses as diagnostics rather than the beta primary objective. | beta amplitude correction |
| 2026-05-18 Asia/Shanghai | `scripts/train_transformer_physical_params_torch.py` | Add alpha-relative no-regression guard | Add beta guards that compare alpha+beta against alpha-only over sliding response windows: response MSE must not exceed alpha by more than `beta_alpha_response_ratio_limit`, zero-lag correlation must not drop beyond `beta_alpha_corr_drop_tol`, and amplitude log error must not worsen beyond `beta_alpha_amp_worsen_tol`. | preserve alpha frequency/phase behavior |
| 2026-05-18 Asia/Shanghai | `scripts/train_transformer_physical_params_torch.py` | Improve beta diagnostics and startup consistency | Add `x/y/tip_y/last5_y_ratio_to_alpha` and `beta_amp_*` / `beta_alpha_guard_*` diagnostics to `training_history.csv`; epoch printouts switch to `_to_alpha` ratios when beta alpha-reference mode is active. Also expose `--load-spectral-feature-dim` so beta warm starts can keep MAIN-ALPHA spectral dimensions when the same frequency-band config is used. | beta smoke-test interpretability |

---

## 32. 2026-05-18 beta damping gain and alpha-relative improvement update

| Time | Files | Required Note | Content | Stage |
|---|---|---|---|---|
| 2026-05-18 Asia/Shanghai | `src/student/transformer/physical_templates.py`, `scripts/train_transformer_physical_params_torch.py` | Add beta damping template gain | Add `beta_damp_template_gain_x/y` to scale `C_x_template` and `C_y_template` as `gain * damping_scale * K_template`. Defaults are `1.0` for compatibility, while beta amplitude experiments can use larger gains so C-target beta rows have enough physical leverage to move high-frequency response amplitudes. | beta amplitude correction v3 |
| 2026-05-18 Asia/Shanghai | `scripts/train_transformer_physical_params_torch.py` | Add beta amplitude improvement objective | Extend the alpha-relative beta amplitude loss with a hinge term that penalizes windows where alpha+beta is not closer to teacher RMS amplitude than alpha-only. Add `--w-beta-amp-improvement`, `--beta-amp-improvement-margin`, and weak damping-sign controls `--w-beta-damp-sign-x/y`, `--beta-damp-sign-min-alpha-error` to discourage negative damping when alpha already has too-large RMS amplitude. | beta amplitude correction v3 |
| 2026-05-18 Asia/Shanghai | `scripts/train_transformer_physical_params_torch.py` | Add beta-specific checkpoint score | Add `--best-score-mode beta_amp`, combining alpha-relative x/y/tip_y/last5_y ratios with beta guard and improvement diagnostics. This selects beta checkpoints for amplitude improvement while keeping alpha no-regression constraints active. | beta checkpoint selection |
| 2026-05-18 Asia/Shanghai | `scripts/evaluate_transformer_vs_baselines.py` | Add alpha-beta difference diagnostics | Four-curve evaluation now writes alpha+beta minus alpha response RMS/max diagnostics, late RMS amplitude ratios versus teacher, and beta theta statistics into `metrics.csv` and `metrics_summary.json`, making near-overlap cases visible numerically. | beta result interpretation |

---

## 33. 2026-05-18 high-frequency beta damping mechanism update
