...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Constant alpha_xy sensitivity scan. It runs teacher/static baseline once, then "
            "rolls out K_eff = K0 + alpha_xy * K_xy_template for several constant alpha_xy values."
        )
    )
    parser.add_argument("--load-file", type=str, default=str(PROJECT_ROOT / "data" / "load" / "test" / "test_complex_case_1.dat"))
    parser.add_argument("--output-dir", type=str, default=str(PROJECT_ROOT / "results" / "transformer" / "alpha_xy_constant_scan"))
    parser.add_argument("--case-name", type=str, default="alpha_xy_constant_scan")
    parser.add_argument("--alpha-xy-values", type=str, default="-0.03,-0.02,-0.01,0.0,0.01,0.02,0.03")
...
def run_constant_alpha_xy_rollout(
    *,
    ...
    theta = torch.as_tensor([float(alpha_xy)], dtype=dtype, device=device)
    ...
    return {
        ...
        "theta": np.full((int(F.shape[0]), 1), float(alpha_xy), dtype=np.float64),
    }
...
def main() -> None:
    ...
    alpha_values = parse_float_list(args.alpha_xy_values)

    print()
    print("=" * 100)
    print("[Scan] Constant alpha_xy phase sensitivity")
    print("=" * 100)
    print(f"  load_file  = {load_file}")
    print(f"  output_dir = {output_dir}")
    print(f"  alpha_xy_values = {alpha_values}")
    ...
    # 3. Build alpha_xy physical core
    print("\n[3/5] Build alpha_xy physical core")
    registry = build_physical_parameter_registry(enabled_params="alpha_xy")
    ...
    for alpha in alpha_values:
        print(f"  alpha_xy={alpha:+.6f}")
        out = run_constant_alpha_xy_rollout(
            core=core,
            F_time=F_time,
            u0=u_static[0],
            v0=v_static[0],
            a0=a_static[0],
            alpha_xy=float(alpha),
            device=device,
            dtype=dtype_core,
        )
        u_model = out["u_full"]
        ...
        metrics = compute_metrics(name=f"alpha_xy_{alpha:+.6f}", u=u_cmp, u_teacher=u_teacher_cmp, n_nodes=n_nodes)
        row: dict[str, Any] = {"model": f"alpha_xy_{alpha:+.6f}", "alpha_xy": float(alpha), **metrics}
    ...
    # 5. Save
    print("\n[5/5] Save artifacts")
    save_csv_union(output_dir / "alpha_xy_constant_scan_metrics.csv", rows)
    save_json(
        output_dir / "alpha_xy_constant_scan_summary.json",
        {
            "args": vars(args),
            "cfg": cfg.__dict__,
    ...
    if u_scan_cache:
        np.savez_compressed(
            output_dir / "alpha_xy_constant_scan_responses.npz",
            time=time,
    ...
    if bool(args.save_plots):
        ...
        for alpha in selected_alphas:
            ...
            plot_timeseries(
                ...
                transformer=u_alpha,
                idx=tip_x,
                title=f"Tip x displacement: constant alpha_xy={alpha:+.4f}",
                ylabel="tip ux",
            ...
            plot_timeseries_error_to_teacher(
                ...
                transformer=u_alpha,
                idx=tip_x,
                title=f"Tip x error: constant alpha_xy={alpha:+.4f}",
                ylabel="tip ux error",
            ...
            plot_timeseries(
                ...
                transformer=u_alpha,
                idx=tip_y,
                title=f"Tip y displacement: constant alpha_xy={alpha:+.4f}",
                ylabel="tip uy",
            ...
            plot_timeseries_error_to_teacher(
                ...
                transformer=u_alpha,
                idx=tip_y,
                title=f"Tip y error: constant alpha_xy={alpha:+.4f}",
                ylabel="tip uy error",
    ...
    print(f"\nmetrics_csv = {output_dir / 'alpha_xy_constant_scan_metrics.csv'}")
    print(f"summary_json = {output_dir / 'alpha_xy_constant_scan_summary.json'}")
    print("✅ PASS: constant alpha_xy scan completed.")
